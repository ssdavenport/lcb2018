...
edible3Solid$time = ymd(as.character(edible3Solid$time))
edible3Liquid$time = ymd(as.character(edible3Liquid$time))

cName = c("Time","Usable Marijuana \n \n","Marijuana Extract \nfor Inhalation\n","Solid Marijuana \nInfused Edibles\n",
          "Liquid Marijuana \nInfused Edibles \n","Other Product \n \n")

#except prediction
caulkins = as.data.frame(dplyr::left_join(x=usableAllFinalResults,y=extractAll,by="time"))
caulkins = as.data.frame(dplyr::left_join(x=caulkins,y=edible3Solid,by="time"))
caulkins = as.data.frame(dplyr::left_join(x=caulkins,y=edible3Liquid,by="time"))
caulkins = as.data.frame(dplyr::left_join(x=caulkins,y=nonOther4[,c("time","other_t_by_smie")],by="time"))
names(caulkins) = cName

#only prediction
caulkinsPred = as.data.frame(dplyr::inner_join(x= pres[24:25,],y=nonOther4_2017[11:12,c("time","other_t_by_smie")],by="time"))

for(i in 2:5){ #scale to grams
  caulkinsPred[,i] = caulkinsPred[,i]*1000000
}

names(caulkinsPred) = cName

#merge

caulkins = rbind(caulkins,caulkinsPred)


#move to plotting
areaPlot2 = melt(caulkins, id.vars ="Time",measure.vars=names(caulkins)[2:6])
areaPlot2[is.na(areaPlot2)] = 0

names(areaPlot2) = c("Time","Inventory_Type","Value")
areaPlot2$Value = as.numeric(areaPlot2$Value)/1000000

zzz = ggplot() + geom_area(data=areaPlot2, aes(x=Time, y=Value, group =Inventory_Type, fill=Inventory_Type)) +
  ylab("Metric Tons of Total THC Per Month") +
  ggtitle("Monthly Weight of THC is Rapidly Increasing \n With Time In Washington State Since Legalization")  +
  geom_vline(xintercept=as.numeric(areaPlot2$Time[193]), linetype=4) +
  theme(text = element_text(size = 30)) +
  theme(plot.title = element_text(hjust = 0.5,vjust = 1)) +
  theme(legend.key = element_rect(size = 5),
    legend.key.size = unit(2, 'lines'),
    legend.spacing.x = unit(1, 'lines')) +
  scale_fill_discrete(name = "Inventory Type \n")
zzz


#### slide plots ####
PRODUCT_GROUP = as.factor(c(rep("THC_SOLID",4),rep("THC_LIQUID",4)))
CV = c(683.1,496.1,147.2,76.59,550.4,484.6,95.70,74.09)
MODEL = as.factor(c(rep(c("Sample Means Based Estimation",
          "Regression with Numeric Variables",
          "Random Forest with Numeric Variables",
          "Random Forest with Text and Numeric Varables"),2)))

CV_PLOTTER = data.frame(PRODUCT_GROUP=PRODUCT_GROUP,
                        CV=CV,
                        MODEL=MODEL)
...
CV_PLOTTER$PRODUCT_GROUP =factor(CV_PLOTTER$PRODUCT_GROUP,levels = levels(CV_PLOTTER$PRODUCT_GROUP)[c(2,1)])
CV_PLOTTER$MODEL = factor(CV_PLOTTER$MODEL, levels = levels(CV_PLOTTER$MODEL)[c(2,1,3,4)])

#library(reshape2)
#CV_PLOTTER_2 <- melt(CV_PLOTTER, id.vars = c("PRODUCT_GROUP", "MODEL"))

ylabelr = "Mean  Error (mg THC)"

#plot
library("ggthemes")
CV_PLOTTER$prod.label <- plyr::mapvalues(CV_PLOTTER$PRODUCT_GROUP, from = c("THC_SOLID", "THC_LIQUID"), to = c("Solid", "Liquid"))
p <- ggplot(data = CV_PLOTTER, aes(x = MODEL, y = CV, group = PRODUCT_GROUP, fill = MODEL))+
  geom_bar(stat = "identity", width = 1, position = "dodge")+
  facet_grid(. ~ prod.label)+
  theme_bw()+
  coord_flip() +
  ggthemes::scale_fill_tableau("tableau10medium") +
  ylab(ylabelr) + labs(x = "") + 
  theme(legend.position = "", text = element_text(size = 30))
p

p2 <- ggplot(data = CV_PLOTTER, aes(x = rev(MODEL), y = CV, group = PRODUCT_GROUP, fill = MODEL))+
  geom_bar(stat = "identity", width = 1, position = "dodge")+
  facet_grid(. ~ prod.label)+
  theme_bw()+
  ggthemes::scale_fill_tableau("tableau10medium") +
  labs(x = "", y = ylabelr) + theme(legend.position = c(0.75, 0.85),
                                    text = element_text(size = 50),
                                    axis.text.x = element_blank(),
                                    legend.text = element_text(size = 25),
                                    legend.title = element_blank()) +
  guides(fill=guide_legend(reverse=TRUE,keywidth=0.5 ,keyheight=0.5,default.unit="inch"), 
...
#Import monthly production data
monthly_production <- read.csv('KW_monthly_production.csv')

#remove cummulative
monthly_production = monthly_production[monthly_production$licensetype!=4,]

#add in time
monthly_production$time = mdy(paste0(substr(as.character(monthly_production$label),1,3)," 1, 20",substr(as.character(monthly_production$label),5,6)))

#rename
names(monthly_production)[5] = "License_Type"

monthly_production$License_Type = as.factor(monthly_production$License_Type)

monthly_production$License_Type = revalue(monthly_production$License_Type, c("1"="2,000", 
                                           "2"="10,000",
                                           "3"="30,000"))
#issdp
sum(monthly_production[monthly_production$time >= as.Date("2016-07-01") & monthly_production$time <= as.Date("2017-06-30"),"dry_month"])

#2016
sum(monthly_production[monthly_production$time >= as.Date("2016-01-01") & monthly_production$time <= as.Date("2016-12-30"),"dry_month"])


#Plot monthly dry flower production
kw = ggplot(monthly_production, 
            aes(x = time, 
                y = dry_month / 1000000, 
                group = License_Type,
                fill = License_Type)) + 
  ...
  xlab("") +
  ylab("Dry Flower (metric tons)") +
  theme_minimal() + 
  theme(text = element_text(size=40), 
        legend.title =element_text("Licensed Canopy (sq.ft.)") ) +
  theme(legend.position="bottom") + 
  geom_vline(xintercept=as.numeric(as.Date("2016-01-01")), linetype=2, size=1.5) +
  geom_vline(xintercept=as.numeric(as.Date("2016-12-30")), linetype=2, size=1.5) +
  ggthemes::scale_fill_tableau("tableau10medium",name = "License Type (square feet)")

kw

#price plot 
pricePlotData = nonOther4[,c("time","um_price","ex_price","smie_price","lmie_price","other_price")]
namerPP = c("Time",
                     ...
                     "Other Product")
names(pricePlotData) = namerPP

pricePlot = melt(pricePlotData, id.vars ="Time",measure.vars=names(pricePlotData)[2:6])
pricePlot[is.na(pricePlot)] = 0

names(pricePlot) = c("Time","Inventory_Type","Expenditure")
pricePlot$Expenditure = pricePlot$Expenditure/1000000

aaa = ggplot() + geom_area(data=pricePlot, aes(x=Time, y=Expenditure, group =Inventory_Type, fill=Inventory_Type)) +
  ylab("Expenditure Per Month (Millions)") +
  ggtitle("Monthly Expenditure is Rapidly Increasing \n With Time In Washington State Since Legalization")  +
  geom_vline(xintercept=as.numeric(areaPlot2$Time[193]), linetype=4) +
  theme(text = element_text(size = 30)) +
  theme(plot.title = element_text(hjust = 0.5,vjust = 1)) +
  theme(legend.key = element_rect(size = 5),
