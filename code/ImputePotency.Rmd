...
divisor <- 2 # scaleability parameter; sets sample size.

# all-time - These row counts come from the entire SQL table (not just what is imported to R)
...
estTHC_RFcv <- function(data, n_pc, n_cv_folds, n_tree, cv = FALSE) {
  ...
  n_pop <- ifelse(unique(data$inventorytype)==23, 
  ...
  output_results$alltime <-
    c('samplekgTHC' = sum(test$pred, na.rm=T) / 1e6, # estimated in sample (all THC > CBD from 100k query)
      'populationkgTHC' = sum(test$pred, na.rm=T) * sample_pop_inflator / 1e6, # after scaling up (assumes no CBD>THC, which must be fixed)
