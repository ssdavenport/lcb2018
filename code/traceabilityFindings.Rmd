...
(Gather this from Kee Won's data)  
...
# Chart production of ingredients (later, repeat annually)
convChild %>%
...
convChild %>%
  ...
  select(-G) %>%
  arrange(-KG) %>% 
   knitr::kable(caption='Ingredient Production of "Non-Flower" Types -Repeat for separate years?', digits=2)
...
Query the inventorytransfers table; group by inventorytype; use inventoryID cutoffs

1.	$ for non-processed outputs (bulk flower / OPM / Wholesale Mix)
2.	$ for processed outputs (usable MJ / BHO / CO2 Hash Oil / etc)
