...
```{r messySmoothingFunctionspotencydata}
...
### Lots and Ingredients

Below we compute the simple average of THC/CBD concentrations among tested products.  Note: CBD was not tracked well in 2014/2015.
...
potencyAnnual %>% 
  ...
  kable(digits=2, caption='Average CBD %s', align="lcccccccccc")
```

```{r}
potencyMonthly %>% 
  filter(!invtype%in%c(retailOther, retailFlower)) %>%
  ggplot(aes(x=date, y=TotalTHCs, color=invtype)) + geom_point() + geom_line() +
  labs(title="Bulk/Ingredient Products", y='THC%')

potencyMonthly %>% 
  filter(!invtype%in%c(retailOther, retailFlower)) %>%
  ggplot(aes(x=date, y=TotalCBDs, color=invtype)) + geom_point() + geom_line() +
  labs(title="Bulk/Ingredient Products", y='CBD%')
```
...
potencyMonthly %>% 
  filter(invtype%in%c(retailOther, retailFlower)) %>%
  ggplot(aes(x=date, y=TotalTHCs, color=invtype)) + geom_point() + geom_line() +
  labs(title="Retail Products", y='THC%')

potencyMonthly %>% 
  filter(invtype%in%c(retailOther, retailFlower)) %>%
  ggplot(aes(x=date, y=TotalCBDs, color=invtype)) + geom_point() + geom_line() +
  labs(title="Retail Products", y='CBD%')
```
...
transfersYrMo %>%
  filter(inventorytype %in%c(plants, lots)) %>%
  ggplot(aes(x=date, y=kUSD, color=inventorytype)) + geom_line() + 
...
edible_thc %>% ggplot(aes(x=date, y=kgTHC, fill=invtype)) + geom_col() + 
...
convChildTYrMo %>%
  filter(inventorytype%in%lots) %>%
  ggplot(aes(x=date, y=childwt/1e6, color=inventorytype)) + geom_line() + geom_point() +
  labs(title="Lots Produced",
       y='1000 KG ("weight")')
```
