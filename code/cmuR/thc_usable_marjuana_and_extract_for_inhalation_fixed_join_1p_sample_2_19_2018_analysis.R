...
thcDensity = ggplot(data=thcRaw[thcRaw$D_INVENTORYTYPE==28,], aes(x=P_VALUE)) +
  ...
  ggtitle("Washington State, Usable Marijuana and Marijuana for Extract: Total THC(%) Density from October 1,2015 - March 30,2017") +
  theme(plot.title = element_text(size=20, hjust = 0.5),axis.title.x = element_text(size=15), axis.title.y = element_text(size=15)) +
  xlab("Total THC(%)") +scale_fill_brewer(palette="Spectral") +
  ylab("Density") #+
...
averagePlot = ggplot(data = averageFrame, aes(x=time, colour=as.factor(D_INVENTORYTYPE),  group=D_INVENTORYTYPE)) +
  geom_rect(data = averageFrame, mapping=aes(ymin= 0,
  ...
  ggtitle("Washington State, Usable Marijuana and Extract for Inhalation: Mean Total THC(%) from October 1,2015 - March 30,2017") +
  theme(plot.title = element_text(size=20, hjust = 0.5),axis.title.x = element_text(size=15), axis.title.y = element_text(size=15)) +
  xlab("Date") +
  ylab("Monthly Mean of THC Concentration") 
