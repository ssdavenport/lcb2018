#clear
rm(list=ls())

#setwd
setwd("~/CMU_Y2/RAND")

#imports
library(plyr)
library(dplyr)
library(lubridate)
library(ggplot2)

#import data ####

#name of data
dataName ='mgturner_one_percent_24_and_28.csv'

#data
thcOverall = read.csv(dataName)

print(dim(thcOverall))
print(summary(as.factor(thcOverall$d_INVENTORYTYPE)))

thcRaw = thcOverall[thcOverall$d_INVENTORYTYPE==24,]
thcExtract = thcOverall[thcOverall$d_INVENTORYTYPE==24,]

#thcRaw = 28 = useable marijuana
#thcExtract = 24 = marijuana extract for inhalation

# D = from dispensing table, P = from sample_potency table

#D_ID = integer
#D_SESSIONTIME = integer
#D_TRANSACTIONID = integer
#D_PRICE = numeric, float
#D_USABLEWEIGHT = numeric, float
#D_INVENTORYTYPE = integer, all 28 = useable marjuana 
#P_NAME = factor, all THC 
#P_Value = numeric, float
#P_LAB_PROVIDED = integer

#USABLE MARIJUANA ##############################

#rename for standardized all caps

names(thcRaw) = toupper(names(thcRaw))

#drop P_NAME and D_INVENTORYTYPE - no information
thcRaw = thcRaw[,(!(names(thcRaw) %in% c("P_NAME","D_INVENTORYTYPE")))]

#convert D_sessiontime to mdy_hms - time originally in Unix 32-bit integer timestamp
thcRaw$D_SESSIONTIME = as.POSIXct(as.numeric(as.character(thcRaw$D_SESSIONTIME)),origin="1970-01-01")

#adds columns for year,month,week
thcRaw$YEAR = year(thcRaw$D_SESSIONTIME)
thcRaw$MONTH = month(thcRaw$D_SESSIONTIME)
thcRaw$WEEK = week(thcRaw$D_SESSIONTIME)

#data alterations ####

#implement price adjustment - for loop only used as progress indicator - mapply works by itself
#setup information

taxConversionTime = ymd_hms("2015-07-01 00:00:00")
thcRaw$D_PRICE <- ifelse(thcRaw$D_SESSIONTIME>=taxConversionTime,thcRaw$D_PRICE*1.37,thcRaw$D_PRICE)

#outlier removal
#identify prices that are in the top and bottom quartile for each month and year

#idnetifies months and years of service
outlierFrame = as.data.frame(unique(thcRaw[,c('YEAR','MONTH')]))
outlierFrame = outlierFrame[ order(outlierFrame[,'YEAR'], outlierFrame[,'MONTH']), ]
rownames(outlierFrame) <- NULL
outlierFrame$one_q = 0
outlierFrame$ninenine_q = 0

#populates with 1% and 99% thresholds 
for(i in 1:nrow(outlierFrame)){
  outlierFrame[i,c('one_q','ninenine_q')] = (thcRaw %>% 
    select(YEAR,MONTH,D_PRICE) %>%
    filter(YEAR==outlierFrame$YEAR[i] & MONTH==outlierFrame$MONTH[i]) %>%
    summarise(one = quantile(x=D_PRICE,probs = c(0.01)),
              ninenine = quantile(x=D_PRICE,probs = c(0.99))))
}

#purges data frame of rows that are above and threshold 
for(i in 1:nrow(outlierFrame)){
  thcRaw = thcRaw[!(thcRaw$YEAR == outlierFrame$YEAR[i] & thcRaw$MONTH == outlierFrame$MONTH[i] & (thcRaw$D_PRICE <= outlierFrame$one_q[i]| thcRaw$D_PRICE >= outlierFrame$ninenine_q[i])),]
}

#price per gram generation
thcRaw$PRICE_PER_GRAM = thcRaw$D_PRICE/thcRaw$D_USABLEWEIGHT

#eliminate rows of THC that are too high - 100 for now, but remove the rest
thcRaw = thcRaw[thcRaw$P_VALUE <=100.00,]

#graphics

#thcRaw - thc density
thcDensity = ggplot(data=thcRaw, aes(x=P_VALUE)) +
  geom_density(fill="darkgreen") +
  ggtitle("Washington State, Extract for Inhalation: Average THC(%) Density from October 1,2015 - March 30,2017") +
  theme(plot.title = element_text(size=30, hjust = 0.5),axis.title.x = element_text(size=15), axis.title.y = element_text(size=15)) +
  xlab("Average THC(%)") +
  geom_vline(xintercept = mean(thcRaw$P_VALUE))

thcDensity

#Total THC concentration / Total Useable Weight

ratioTHCFrame = thcRaw %>%
  select(YEAR,MONTH,P_VALUE,D_USABLEWEIGHT) %>%
  group_by(YEAR,MONTH) %>%
  summarise(monthRatio = sum(P_VALUE)/sum(D_USABLEWEIGHT))

ratioTHCFrame = as.data.frame(ratioTHCFrame)
ratioTHCFrame$time = lubridate::ymd(paste0(ratioTHCFrame$YEAR,"/",ratioTHCFrame$MONTH,"/01"))


ratioPlot = ggplot(data = ratioTHCFrame, aes(x=time, y=monthRatio)) +
  geom_rect(data = ratioTHCFrame, mapping=aes(ymin= 0,
                                              ymax = max(averageTHCFrame$monthRatio),
                                              xmin=lubridate::ymd("2015-08-01"), xmax=lubridate::ymd("2015-11-01"), 
                                              fill="2015/08/01\n        -      \n2015/11/01"), alpha=0.25)+
  geom_line(size=3) +
  ggtitle("Washington State, Usable Marijuana: Average THC(%) to Usable Weight Ratio from October 1,2015 - March 30,2017") +
  theme(plot.title = element_text(size=25, hjust = 0.5),axis.title.x = element_text(size=15), axis.title.y = element_text(size=15)) +
  xlab("Date") +
  ylab("Monthly Weighted Total of THC Concentration, Weighted By Total Usable Weight") +
  guides(fill=guide_legend(title="Missing Data"))
  
ratioPlot

#for each product, THC concentration/ Total Useable Weight
thcRaw2 = thcRaw

averageTHCFrame = thcRaw2 %>%
  select(YEAR,MONTH,D_USABLEWEIGHT) %>%
  group_by(YEAR,MONTH) %>%
  summarise(THC_MONTH_TOTAL = sum(D_USABLEWEIGHT))

thcRaw2$THC_MONTH_TOTAL=0

for(i in 1:nrow(averageTHCFrame)){
  thcRaw2[thcRaw2$MONTH==averageTHCFrame$MONTH[i] & thcRaw2$YEAR==averageTHCFrame$YEAR[i], 'THC_MONTH_TOTAL'] <- averageTHCFrame$THC_MONTH_TOTAL[i]
}

thcRaw2$weightedAverage = (thcRaw2$P_VALUE*(thcRaw2$D_USABLEWEIGHT/thcRaw2$THC_MONTH_TOTAL))


averageTHCFrame = thcRaw2 %>%
  select(YEAR,MONTH,weightedAverage) %>%
  group_by(YEAR,MONTH) %>%
  summarise(THC_MONTH_TOTAL = sum(weightedAverage))

averageTHCFrame$time = lubridate::ymd(paste0(averageTHCFrame$YEAR,"/",averageTHCFrame$MONTH,"/01"))

averagePlot = ggplot(data = averageTHCFrame, aes(x=time, y=THC_MONTH_TOTAL)) +
  geom_rect(data = averageTHCFrame, mapping=aes(ymin= 0,
                                              ymax = max(averageTHCFrame$THC_MONTH_TOTAL),
                                              xmin=lubridate::ymd("2015-08-01"), xmax=lubridate::ymd("2015-11-01"), 
                                              fill="2015/08/01\n        -      \n2015/11/01"), alpha=0.25)+
  geom_point(size=4) +
  geom_line(size=2.5) +
  ggtitle("Washington State, Extract for Inhalation: Average THC(%) to Usable Weight Mean from October 1,2015 - March 30,2017") +
  theme(plot.title = element_text(size=25, hjust = 0.5),axis.title.x = element_text(size=15), axis.title.y = element_text(size=15)) +
  xlab("Date") +
  ylab("Monthly Sum of (Item's THC Concentration * Item's Usable Weight)/Sum(All Items in Month's Usable Weight)") +
  guides(fill=guide_legend(title="Missing Data"))

averagePlot

#MARIJUANA FOR EXTRACT ##############################

#rename for standardized all caps

names(thcExtract) = toupper(names(thcExtract))

#drop P_NAME and D_INVENTORYTYPE - no information
thcExtract = thcExtract[,(!(names(thcExtract) %in% c("P_NAME","D_INVENTORYTYPE")))]

#convert D_sessiontime to mdy_hms - time originally in Unix 32-bit integer timestamp
thcExtract$D_SESSIONTIME = as.POSIXct(as.numeric(as.character(thcExtract$D_SESSIONTIME)),origin="1970-01-01")

#adds columns for year,month,week
thcExtract$YEAR = year(thcExtract$D_SESSIONTIME)
thcExtract$MONTH = month(thcExtract$D_SESSIONTIME)

#data alterations ####

#implement price adjustment - for loop only used as progress indicator - mapply works by itself
#setup information

taxConversionTime = ymd_hms("2015-07-01 00:00:00")
thcExtract$D_PRICE <- ifelse(thcExtract$D_SESSIONTIME>=taxConversionTime,thcExtract$D_PRICE*1.37,thcExtract$D_PRICE)

#outlier removal
#identify prices that are in the top and bottom quartile for each month and year

#idnetifies months and years of service
outlierFrame = as.data.frame(unique(thcExtract[,c('YEAR','MONTH')]))
outlierFrame = outlierFrame[ order(outlierFrame[,'YEAR'], outlierFrame[,'MONTH']), ]
rownames(outlierFrame) <- NULL
outlierFrame$one_q = 0
outlierFrame$ninenine_q = 0

#populates with 1% and 99% thresholds 
for(i in 1:nrow(outlierFrame)){
  outlierFrame[i,c('one_q','ninenine_q')] = (thcExtract %>% 
                                               select(YEAR,MONTH,D_PRICE) %>%
                                               filter(YEAR==outlierFrame$YEAR[i] & MONTH==outlierFrame$MONTH[i]) %>%
                                               summarise(one = quantile(x=D_PRICE,probs = c(0.01)),
                                                         ninenine = quantile(x=D_PRICE,probs = c(0.99))))
}

#purges data frame of rows that are above and threshold 
for(i in 1:nrow(outlierFrame)){
  thcExtract = thcExtract[!(thcExtract$YEAR == outlierFrame$YEAR[i] & thcExtract$MONTH == outlierFrame$MONTH[i] & (thcExtract$D_PRICE <= outlierFrame$one_q[i]| thcExtract$D_PRICE >= outlierFrame$ninenine_q[i])),]
}

#price per gram generation
thcExtract$PRICE_PER_GRAM = thcExtract$D_PRICE/thcExtract$D_USABLEWEIGHT

#eliminate rows of THC that are too high - 100 for now, but remove the rest
thcExtract = thcExtract[thcExtract$P_VALUE <=100.00,]

#graphics

#thcExtract - thc density
thcDensityExtract = ggplot(data=thcExtract, aes(x=P_VALUE)) +
  geom_density(fill="darkgreen") +
  ggtitle("Washington State, Marijuana for Inhalation: Average THC(%) Density from October 1,2015 - March 30,2017") +
  theme(plot.title = element_text(size=30, hjust = 0.5),axis.title.x = element_text(size=15), axis.title.y = element_text(size=15)) +
  xlab("Average THC(%)") +
  geom_vline(xintercept = mean(thcExtract$P_VALUE))

thcDensityExtract

#Total THC concentration / Total Useable Weight
ratioTHCFrame = thcExtract %>%
  select(YEAR,MONTH,D_PRICE,D_USABLEWEIGHT) %>%
  group_by(YEAR,MONTH) %>%
  summarise(monthRatio = sum(D_PRICE)/sum(D_USABLEWEIGHT))

ratioTHCFrame = as.data.frame(ratioTHCFrame)
ratioTHCFrame$time = lubridate::ymd(paste0(ratioTHCFrame$YEAR,"/",ratioTHCFrame$MONTH,"/01"))


ratioPlot = ggplot(data = ratioTHCFrame, aes(x=time, y=monthRatio)) +
  geom_rect(data = ratioTHCFrame, mapping=aes(ymin= 0,
                                              ymax = max(averageTHCFrame$monthRatio),
                                              xmin=lubridate::ymd("2015-08-01"), xmax=lubridate::ymd("2015-11-01"), 
                                              fill="2015/08/01\n        -      \n2015/11/01"), alpha=0.25)+
  geom_line(size=3) +
  ggtitle("Washington State, Marijuana for Inhalation: Average THC(%) to Usable Weight Ratio from October 1,2015 - March 30,2017") +
  theme(plot.title = element_text(size=25, hjust = 0.5),axis.title.x = element_text(size=15), axis.title.y = element_text(size=15)) +
  xlab("Date") +
  ylab("Monthly Total THC Concentration/Total Usable Weight") +
  guides(fill=guide_legend(title="Missing Data"))

ratioPlotExtract

#for each product, THC concentration/ Total Useable Weight
thcExtract2 = thcExtract
thcExtract2$itemAverage = thcExtract2$P_VALUE/thcExtract2$D_USABLEWEIGHT

averageTHCExtractFrame = thcExtract2 %>%
  select(YEAR,MONTH,D_PRICE,itemAverage) %>%
  group_by(YEAR,MONTH) %>%
  summarise(monthRatio = mean(itemAverage))

averageTHCExtractFrame = as.data.frame(averageTHCExtractFrame)
averageTHCExtractFrame$time = lubridate::ymd(paste0(averageTHCExtractFrame$YEAR,"/",averageTHCExtractFrame$MONTH,"/01"))

averagePlotExtract = ggplot(data = averageTHCExtractFrame, aes(x=time, y=monthRatio)) +
  geom_rect(data = averageTHCExtractFrame, mapping=aes(ymin= 0,
                                                ymax = max(averageTHCExtractFrame$monthRatio),
                                                xmin=lubridate::ymd("2015-08-01"), xmax=lubridate::ymd("2015-11-01"), 
                                                fill="2015/08/01\n        -      \n2015/11/01"), alpha=0.25)+
  geom_point(size=4) +
  geom_line(size=2.5) +
  ggtitle("Washington State, Marijuana for Inhalation: Average THC(%) to Usable Weight Mean from October 1,2015 - March 30,2017") +
  theme(plot.title = element_text(size=25, hjust = 0.5),axis.title.x = element_text(size=15), axis.title.y = element_text(size=15)) +
  xlab("Date") +
  ylab("Monthly Mean of THC Concentration/Usable Weight for all Items") +
  guides(fill=guide_legend(title="Missing Data"))

averagePlotExtract